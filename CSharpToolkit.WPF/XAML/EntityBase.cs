namespace CSharpToolkit.XAML {
    ...
    public abstract class EntityBase : IUserNotifier, INotifyDataErrorInfo, INotifyPropertyChanged, IDataErrorInfo, IExplicitErrorAdder, INotifyDisposable, IFocusChanger {
        ...
        public EntityBase() {
            ...
            PropertyStore.PropertyChanged += (s, e) => {
                if (_ignoreList.Contains(e.PropertyName) == false)
        ...
        protected PropertyStore PropertyStore { get; } = new PropertyStore();

        protected void UpdatePropertyValueNoPropertyChanged<TValue>(TValue value, [CallerMemberName]string propertyName = "") {
            _ignoreList.Add(propertyName);
            PropertyStore.SetValue(propertyName, value);
            _ignoreList.Remove(propertyName);
        }
        ...
        public IEnumerable GetErrors(string propertyName) =>
            string.IsNullOrEmpty(propertyName) ?
                _publicErrors.Select(e => e.Item2) :
                ((IEnumerable)_publicErrors.FirstOrDefault(e => e.Item1 == propertyName)?.Item2);
        ...
        /// <summary>
        /// Runs validation for speciified property by passing to indexer.
        /// </summary>
        /// <param name="propertyName">The property for which the validation should be forced.</param>
        public void ForceValidationForProperty(string propertyName) {
            var tmp = this[propertyName];
        }
