namespace CSharpToolkit.DataAccess.Test {
    ...
    public class DataReaderStub : IDataReader {
        ...
        List<OrderedDictionary> _results;
        OrderedDictionary _result;
        int currentIndex = 0;

        public DataReaderStub(List<OrderedDictionary> records) {
            _results = records;
        }

        public object this[string name] => _result[name];

        public object this[int i] => _result[i];
        ...
        public bool Read() {
            if (currentIndex == _results.Count)
                return false;
            _result = _results[currentIndex++];
            return true;
        }
