namespace CSharpToolkit.Logging {
    ...
    public class WebDataRequestorAsyncLogger : IWebDataRequestorAsync {
        ...
        public Task<OperationResult<byte[]>> MakeWebRequestAsync(string url, IEnumerable<Tuple<string, string>> data, IEnumerable<Tuple<HttpRequestHeader, string>> requestHeaders) {
            ...
            string webRequest = Use.StringBuilder(builder => {
                ...
                if (data.Any()) {
                    ...
                    builder.Append(
                        string.Join("\r\n",
                            data
                            .Where(d => string.IsNullOrWhiteSpace(d.Item2) == false)
                            .Select(d =>
                                $"\t{d.Item1} :\r\n\t\t{string.Join("\\r\\n", d.Item2.Replace("\r\n", "\n").Replace("\r", "\n").Split('\n'))}"
                            )
                        )
            ...
            if (loggingOperation.WasSuccessful) {
                ...
                return Task.FromResult(new OperationResult<byte[]>(loggingOperation.Exceptions));
            }
            return Task.FromResult(new OperationResult<byte[]>(Response));
        }
