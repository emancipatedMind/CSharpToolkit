namespace CSharpToolkit.Utilities {
    ...
    public static class Get {
        ...
        */
        ...
        public static List<string> ObjectStringifiedAsLines(object obj) =>
            Format(obj, 0, true);

        public static List<string> Format(object obj, int precedingSpaceCount) =>
            Format(obj, precedingSpaceCount, true);

        static List<string> Format(object obj, int precedingSpaceCount, bool attachName = true) =>
            PrependSpaceCount(precedingSpaceCount, list => {
                if (obj != null && attachName)
                    list.Add($"Name : {obj.GetType().AssemblyQualifiedName}");

                list.AddRange(GetAllProperties(obj, FormatFilter, 2));

                if (obj is Exception) {
                    list.AddRange(Format(((Exception)obj).InnerException, 2));
                }
                ...
                if (obj is IDictionary) {
                    ...
                    foreach (DictionaryEntry entry in dictionary) {
                        list.Add(new string(' ', 2) + $"[{index++}]");
                        if (entry.Value == null) {
                            list.Add(new string(' ', 4) + $"{entry.Key} : <NULL>");
                            continue;
                        }
                        if (entry.Value is string || entry.Value.GetType().IsSubclassOf(typeof(ValueType))) {
                            list.Add(new string(' ', 4) + $"{entry.Key} : {entry.Value}");
                            continue;
                        }
                        list.Add(new string(' ', 4) + $"{entry.Key} :");
                        list.AddRange(Format(entry.Value, 4, false));
                    }
                }
                else if (typeof(IEnumerable).IsAssignableFrom(obj.GetType()) || obj.GetType().IsArray) {
                    var enumerator = ((IEnumerable)obj).GetEnumerator();
                    int index = 0;
                    while (enumerator.MoveNext()) {
                        list.Add(new string(' ', 2) + $"Name[{index++}] : {enumerator.Current.GetType().AssemblyQualifiedName}");
                        list.AddRange(Format(enumerator.Current, 4, false));
                    }
                }
            });

        static List<string> GetAllProperties(object obj, Func<PropertyInfo, bool> filter, int precedingSpaceCount) =>
            PrependSpaceCount(precedingSpaceCount, list => {
                if (obj == null) {
                    list.Add("<NULL>");
                    return;
                ...
                var props = obj.GetType().GetProperties().Where(filter).ToArray();

                if (props.Any() == false) {

                    if (obj.ToString().Length < 200) {
                        list.Add($"{Regex.Replace(obj.ToString(), @"(\s{2,})|([\r\n]{1,2})", " ")}");
                    }
                    else {
                        list.AddRange(obj.ToString().Split(new[] { "\r\n" }, StringSplitOptions.None).Select(str => "  " + str.Trim()));
                    }
                }

                foreach (PropertyInfo prop in props.OrderBy(PropOrderBy)) {
                    ...
                    else if (value is string || prop.PropertyType.IsSubclassOf(typeof(ValueType)))
                        ProcessNormally(value, prop, list);
                    else if (value is Exception)
                        list.AddRange(Format(((Exception)value), precedingSpaceCount));
                    else if (value is XDocument) {
                        list.Add($"{prop.Name} :");
                        list.AddRange(
                            PrependSpaceCount(2, subList =>
                                subList.AddRange(((XDocument)value).ToString(SaveOptions.None).Split(new[] { "\r\n" }, StringSplitOptions.None))
                            )
                        );
                    }
                    ...
                    if (value is string || value == null) {
                        ...
                        list.Add($"{prop.Name} : {prop.PropertyType.AssemblyQualifiedName}");
        ...
        static void ProcessNormally(object value, PropertyInfo prop, List<string> list) {
            if (value.ToString().Length == 0) {
                list.Add($"{prop.Name} : <EmptyString>");
            }
        ...
        static List<string> PrependSpaceCount(int precedingSpaceCount, Action<List<string>> UseList) =>
        ...
        static bool FormatFilter(PropertyInfo prop) =>
            (
                typeof(Exception).IsAssignableFrom(prop.PropertyType) == false ||
                prop.Name != nameof(Exception.InnerException)
            ) &&
            (
                typeof(IDictionary).IsAssignableFrom(prop.PropertyType) == false ||
                prop.GetIndexParameters().Length == 0
            ) &&
            (
                typeof(IEnumerable).IsAssignableFrom(prop.PropertyType) == false ||
                prop.GetIndexParameters().Length == 0
            ) &&
            (
                prop.PropertyType.IsArray == false ||
                prop.GetIndexParameters().Length == 0
            ) &&
            prop.GetIndexParameters().Length == 0 && prop.CanRead && prop.GetMethod.IsPublic;

        static int PropOrderBy(PropertyInfo prop) {
